  ...
  return (
    <div className="flex gap-3 md:gap-4 overflow-x-auto pb-2 snap-x">
      {columns.map(col => {
        ...
        return (
          <div key={col.status} className="min-w-[260px] md:min-w-0 md:flex-1 snap-start rounded-xl bg-muted/50 p-3 relative overflow-hidden">
            {/* Accent top */}
            ...
            <div className="space-y-2">
              {columnTasks.map(task => {
                ...
                return (
                  <div
                    key={task.id}
                    className="rounded-xl bg-card border border-border p-3.5 hover:bg-muted/50 transition-colors cursor-pointer"
                  >
                    ...
                    {task.description && (
                      <p className="text-[11px] text-muted-foreground mb-2.5 line-clamp-2">{task.description}</p>
                    )}
